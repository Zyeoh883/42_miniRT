...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   scene_objects_utils2.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: zyeoh <zyeoh@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/16 17:42:44 by zyeoh             #+#    #+#             */
/*   Updated: 2026/01/16 17:43:17 by zyeoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
cl_float	get_cl_float(char *str)
{
	...
	if (ft_strlen(str) > LONG_NUM_DIGITS + 1)
		error_and_exit("Float digits must be less than 19 digits",
			EXIT_FAILURE);
	temp = str;
	if (*temp == '-' || *temp == '+')
		temp++;
...
cl_float3	get_dir(char *str)
{
	...
	result = get_cl_float3(str);
	if (result.x == 0 && result.y == 0 && result.z == 0)
		error_and_exit("Direction vector must not be 0,0,0", EXIT_FAILURE);
