...
/*   Updated: 2026/01/09 22:26:48 by zyeoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void fill_gpu_list(t_opencl *opencl, cl_device_id *gpu_list, cl_platform_id *platform_list, cl_uint num_platforms)
{
  cl_uint platform_gpus;
  cl_uint n = -1;
  cl_uint m = 0;
  cl_uint current_idx = 0;

  while (++n < num_platforms)
  { 
    if (clGetDeviceIDs(opencl->platform[n], CL_DEVICE_TYPE_GPU, 0, NULL, &platform_gpus) != CL_SUCCESS)
      perror_and_exit("Platform fetch failed", EXIT_FAILURE);

		if (platform_gpus > 0)
    {
		  if (clGetDeviceIDs(opencl->platform[n], CL_DEVICE_TYPE_GPU, platform_gpus, &gpu_list[current_idx], NULL) != CL_SUCCESS)
...
void user_choose_gpu(t_opencl *opencl, cl_device_id *gpu_list, cl_platform_id *platform_list, cl_uint total_gpus)
{
  ...
  while (++n < total_gpus)
  {
      clGetPlatformInfo(platform_list[n], CL_PLATFORM_NAME, sizeof(platform_name), platform_name, NULL);
      clGetDeviceInfo(gpu_list[n], CL_DEVICE_NAME, sizeof(device_name), device_name, NULL);
