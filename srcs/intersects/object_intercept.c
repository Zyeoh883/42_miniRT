...
/*   Updated: 2026/01/16 17:58:23 by zyeoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//     tmin = _mm_max_ps(_mm_max_ps(tmin, _mm_shuffle_ps(tmin, tmin,
// _MM_SHUFFLE(0, 3, 2, 1))),
//                       _mm_shuffle_ps(tmin, tmin, _MM_SHUFFLE(0, 1, 3, 2)));
//     tmax = _mm_min_ps(_mm_min_ps(tmax, _mm_shuffle_ps(tmax, tmax,
// _MM_SHUFFLE(0, 3, 2, 1))),
//                       _mm_shuffle_ps(tmax, tmax, _MM_SHUFFLE(0, 1, 3, 2)));

//     // Check for intersection
//     __m128 cond = _mm_and_ps(_mm_cmpge_ps(tmax, tmin), _mm_cmpge_ps(tmax,
// _mm_setzero_ps()));
...
{
	return (_mm_max_ps(_mm_max_ps(tmin, _mm_shuffle_ps(tmin, tmin,
					_MM_SHUFFLE(0, 3, 2, 1))), _mm_shuffle_ps(tmin, tmin,
				_MM_SHUFFLE(0, 1, 3, 2))));
}

static __m128	get_max(__m128 tmax)
{
	return (_mm_min_ps(_mm_min_ps(tmax, _mm_shuffle_ps(tmax, tmax,
					_MM_SHUFFLE(0, 3, 2, 1))), _mm_shuffle_ps(tmax, tmax,
				_MM_SHUFFLE(0, 1, 3, 2))));
}
...
int	intersect_aabb(t_ray *ray, __m128 max, __m128 min)
{
	...
	tmin = _mm_div_ps(_mm_set1_ps(1.0f), _mm_blendv_ps(ray->direction,
				_mm_set1_ps(1.0f), is_zero));
	t[0] = _mm_mul_ps(_mm_sub_ps(min, *ray->pos), tmin);
	t[1] = _mm_mul_ps(_mm_sub_ps(max, *ray->pos), tmin);
	tmin = _mm_min_ps(t0, t[1]);
	tmax = _mm_max_ps(t0, t[1]);
	tmin = _mm_blendv_ps(tmin, _mm_set1_ps(-INFINITY), is_zero);
	tmax = _mm_blendv_ps(tmax, _mm_set1_ps(INFINITY), is_zero);
	tmin = get_min(tmin);
	tmax = get_max(tmax);
	cond = _mm_and_ps(_mm_cmpge_ps(tmax, tmin), _mm_cmpge_ps(tmax,
				_mm_setzero_ps()));
	return ((_mm_movemask_ps(cond) & 0x7) == 0x7);
}
...
//     is_zero = _mm_cmpeq_ps(ray->direction, _mm_setzero_ps());
//     is_zero = _mm_div_ps(_mm_set1_ps(1.0f), _mm_blendv_ps(ray->direction,
// _mm_set1_ps(1.0f), is_zero));
